...
public class SRecyclerAdapter<D> extends RecyclerSwipeAdapter<BViewHolder> implements IAdapter<D> {
    ...
    private void bindView(@NonNull BViewHolder holder, int position, @NonNull List<Object> payloads){
        ...
        if (itemPosition < mDatas.size() && mItemConverts.size() > 0){
            Object data = mDatas.get(itemPosition);
            if (data instanceof ItemDelegate) {
                ItemDelegate delegate = (ItemDelegate) data;
                mItemConverts.get(delegate.getItemViewRes()).convert(holder, delegate, itemPosition, payloads);
            } else {
                mItemConverts.get(itemLayoutRes).convert(holder, data, itemPosition, payloads);
            }
            //单击事件
            addClickListener(holder.getConvertView(),itemPosition, (D) data);
            //长按事件
            addLongClickListener(holder.getConvertView(),itemPosition, (D) data);
            return;
        }
    ...
    public int getItemViewType(int position) {
        ...
        if (itemPostion < mDatas.size()){
            ...
            if (itemData instanceof ItemDelegate){
     ...
     * @param mData
     */
    private void addClickListener(View contentView, final int position, final D mData){
     ...
     * @param mData
     */
    private void addLongClickListener(View contentView, final int position, final D mData){
     ...
     */
    public void addContentLayout(@LayoutRes int contentLayoutRes,ItemViewConvert<D> convert, boolean fromXml){
        if (fromXml) {
            itemLayoutRes = contentLayoutRes;
        }
